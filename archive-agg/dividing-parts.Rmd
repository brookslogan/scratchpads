...
Is idea for mem-limited future stream to try to iterate a sliding window of (mem limit? worker limit? something else?) futures and diff just the first two at a time?  Or will that not compose well?  Is it possible to have an adaptive window based on resource usage estimates?  Is it possible to choose between advancing the window or advancing some future dependency based on resource limits?  Any way to make resource balancing shared across ops & compose well?
